...
DATA_FOLDER='./Data/'
F_NAME = 'exp_matrix'
exp_matrix = pd.read_csv('./exp_matrix.csv')

## Generate the regulons
motifs = load_motifs(os.path.join(DATA_FOLDER, '{}.motifs.csv'.format(F_NAME)))
...
sc.pl.rank_genes_groups_heatmap(sc_auc_mtx ,groupby='leiden_cluster',n_genes=5,swap_axes=True, figsize=(6,7),use_raw=False,cmap='RdBu_r')
sc.pl.rank_genes_groups_dotplot(sc_auc_mtx ,groupby='leiden_cluster',n_genes=15, use_raw=False)
